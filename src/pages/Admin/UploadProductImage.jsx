...
import { listProducts, updateProductImagePath } from "../../services/productsService";
...
export default function UploadProductImage() {
  ...
  async function handleUpload(e) {
    e.preventDefault();
    setStatus("");
    if (!productId || !slug || !file) {
      setStatus("Selecciona un producto y archivo");
      return;
    }
    ...
    try {
      ...
      setStatus("Imagen subida, pero falló actualizar el producto: " + err.message);
    }
  }

  async function handleDelete() {
    if (!productId || !slug) return;
    const pathPrefix = `products/${slug}.`;
    // intenta borrar posibles extensiones comunes
    const candidates = ["jpg","jpeg","png","webp","gif","avif"].map(e => `${pathPrefix}${e}`);
  ...
  return (
    <div className="max-w-2xl p-6 space-y-4">
      <h1 className="text-2xl font-semibold">Gestión de imágenes de productos</h1>
